...
public class DoctorService : ServiceBase<Doctor>, IDoctorService
{
    ...
    private async Task AddOrUpdateAsync(DoctorRequest model, CancellationToken cancellationToken = default)
    {
        ...
        if (model.IsNew)
        {
            ...
            await doctorRepository.AddLanguagesToDoctorAsync(doctor, languages, cancellationToken);
        }
        else
        {
            ...
            await doctorRepository.UpdateDoctorLanguagesAsync(doctor, languages, cancellationToken);
        }
    }
}
